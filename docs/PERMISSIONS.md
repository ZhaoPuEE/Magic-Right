...
Opening a target in another app normally uses Launch Services/`NSWorkspace` and does not require Apple Events automation. Known integrations use structured arguments, not UI automation. Magic Right should avoid Apple Events; adding an Apple Events integration later requires a feature-specific explanation and permission review.
...
Onboarding may recommend launch at login, but the user must confirm it. The setting should use the supported macOS service-management API and remain reversible in both Magic Right and System Settings. It does not require administrator privileges.
...
The host app and Finder extension require compatible signing and the same App Group entitlement. Local development needs a team for which Xcode can provision that App Group for both targets; free Personal Team capability support must be confirmed in the generated profiles rather than assumed. A public DMG requires Apple Developer Program membership, Developer ID signing, notarization, and stapling; see [Building and releasing](BUILDING.md).
